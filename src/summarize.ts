...
export const DEFAULT_MAX_ATTRS_PER_ENTRY = 50
...
function stableStringify(v: unknown): string {
  ...
  const out: string[] = []
  const stack: StringifyWork[] = [{ val: v }]
  while (stack.length > 0) {
    const w = stack.pop() as StringifyWork
    if ('lit' in w) {
      out.push(w.lit)
      continue
    }
...
/** One item of `stableStringify`'s explicit work stack: either a value still to
 *  be serialised, or literal text already decided. */
type StringifyWork = { lit: string } | { val: unknown }
...
export function summarize(plan: Plan | null | undefined, options: SummarizeOptions = {}): Result {
  ...
  for (const c of changes) {
    ...
    if (inPlace && ch.before_sensitive == null && ch.after_sensitive == null) unmasked = true
